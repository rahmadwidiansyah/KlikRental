<x-guest-layout>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-4">
        ...
        <div>
            <x-input-label for="name" :value="__('Nama Lengkap')" class="font-inter font-semibold text-[13px] text-on-surface" />
            <x-text-input id="name" class="block mt-1 w-full bg-surface text-on-surface border-outline-variant/60 rounded-xl focus:border-primary focus:ring-primary/20" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>

        <div>
            <x-input-label for="email" :value="__('Email')" class="font-inter font-semibold text-[13px] text-on-surface" />
            <x-text-input id="email" class="block mt-1 w-full bg-surface text-on-surface border-outline-variant/60 rounded-xl focus:border-primary focus:ring-primary/20" type="email" name="email" :value="old('email')" required autocomplete="username" />
            <x-input-error :messages="$errors->get('email')" class="mt-2" />
        </div>

        <div>
            <x-input-label for="phone_number" :value="__('Nomor WhatsApp')" class="font-inter font-semibold text-[13px] text-on-surface" />
            <x-text-input id="phone_number" class="block mt-1 w-full bg-surface text-on-surface border-outline-variant/60 rounded-xl focus:border-primary focus:ring-primary/20" type="text" name="phone_number" :value="old('phone_number')" required placeholder="08..." />
